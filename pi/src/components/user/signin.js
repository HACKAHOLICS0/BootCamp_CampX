...
export default function Signin() {
  ...
  const [recaptchaKey, setRecaptchaKey] = useState("6LeIxAcTAAAAAJcZVRqyHh71UMIEGNQ_MXjiZKhI");
  ...
  const onSubmit = async (e) => {
    ...
    try {
      const response = await fetch("http://localhost:5003/api/auth/signin", {
        method: "POST",
        headers: {
  ...
  const handleGitHubLogin = () => {
    window.location.href = "http://localhost:5003/api/auth/github/callback";
  };
