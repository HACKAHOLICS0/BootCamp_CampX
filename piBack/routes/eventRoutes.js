...
router.get('/admin/check-auth', userAuth, (req, res) => {
...
router.get('/admin/pending', userAuth, eventController.getPendingEvents);
...
router.get('/admin/test-approve/:id', userAuth, async (req, res) => {
    // Vérifier si l'utilisateur est un administrateur
    if (req.user.typeUser !== 'admin') {
...
router.post('/:id/approve', userAuth, eventController.approveEvent);

// Route de test pour rejeter un événement spécifique (admin only)
router.get('/admin/test-reject/:id', userAuth, async (req, res) => {
    // Vérifier si l'utilisateur est un administrateur
    if (req.user.typeUser !== 'admin') {
        return res.status(403).json({ message: 'Admin privileges required' });
    }
...
router.post('/:id/reject', userAuth, eventController.rejectEvent);
